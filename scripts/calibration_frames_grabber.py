...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        if publish:
            try:
                (wMeTrans, wMeRot) = listener.lookupTransform(args.end_effector_frame, args.world_frame, rospy.Time(0))
                (cMoTrans, cMoRot) = listener.lookupTransform(args.object_frame, args.camera_frame, rospy.Time(0))
	    ...
	    stamp = rospy.Time.now()
            ...
            cMo = TransformStamped(header = Header(seq, stamp, args.camera_frame),
                                   child_frame_id = args.object_frame,
                                   transform = Transform(cMoTrans, cMoRot))
            wMe = TransformStamped(header = Header(seq, stamp, args.world_frame),
