...
for i in xrange( max_nb_tries ):
  sample = random.sample( xrange( len( calib.robots ) ), 10)
  
  q_proj, rgb_pose, error = calib.optimize( q_init, sample )

  for robot_id in list( set( xrange( len( calib.robots ) ) ) - set( sample ) ):
    new_sample = sample + [ robot_id ]
    #new_q_proj, new_rgb_pose, new_error = calib.optimize( q_proj, new_sample )
    new_q_proj, new_rgb_pose, new_error = calib.optimize( q_init, new_sample )
