...
for x in [ border + ( i + 0.5 ) * ( image_width - 2 * border ) / nb_cols for i in range( nb_cols ) ]:
  x -= border # The chessboard is on the right of the aluminum plate, so we shift the gaze to the left to see it
  for y in [ border + ( i + 0.5 ) * ( image_height - 2 * border ) / nb_rows for i in range( nb_rows ) ]:
    ...
    while True:
      ...
      ps.createTransformationConstraint ("gaze", "talos/rgbd_rgb_optical_joint", "mire/root_joint", (che, 0, 0.35, 0.0556137029376, 0.989856018959, 0.128925982673, 0.0216856811927), [True,]*6)
      break
